...
class Solution{
    ...
    private boolean isLeftLeaf(Node left){
        if(left==null)
            return false;
    ...
    private void sum(Node root){
        if(root!=null){
            if(isLeftLeaf(root.left)) //this checks for next left node is leaf or not
                res+=root.left.data;
            else
                sum(root.left); //above steps gets repeated for both left and right subtree
            sum(root.right);
        }
    }
}
