...
class Solution
{
    ...
    private static boolean check(int n, int counter){
        ...
        if(n%counter==0)
            return false;
        return check(n-(n/counter), counter+1); //n-n/counter gives number of element remaining after removal of elements by certain counter, this way we can reach till end and find if last element will be deleted or not
    }
