...
// { Driver Code Starts
    ...
    class MinHeap {
        ...
        {   /* using without heapify
            ...
            int temp1=harr[0];
            harr[0]=harr[heap_size-1];
            harr[heap_size-1]=temp1;
            heap_size--;
            int i=0;
            while(i<heap_size){
                int left=2*i+1>=heap_size ? Integer.MAX_VALUE : harr[2*i+1];
                int right=2*i+2>=heap_size ? Integer.MAX_VALUE : harr[2*i+2];
                int large= left<=right ? 2*i+1 : 2*i+2;
               
                if(large<heap_size && harr[i]>harr[large]){
