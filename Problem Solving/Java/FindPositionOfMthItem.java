...
class Solution {
    static int findPosition(int n , int m , int k) {
        // code here
        //n-k+1 is no of positions befroe reaching begigining of circle, if m is less than this value then simply return valu for m-1th position considerirng k
        if(m<=n-k+1)
            return m+k-1;
        //this finds remaining items before we reach begigningin or new cirvle again
        m=m-(n-k+1);
        //doing m%n to skip all completed rounds, if it is 0 then it means position is n or it is m%n
        return m%n==0 ? n : m%n;
    }
};
