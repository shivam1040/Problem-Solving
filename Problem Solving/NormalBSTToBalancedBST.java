...
class GfG
{
    Node buildBalancedTree(Node root) 
    {
        ...
        toList(root, aux);
        return toBalanced(aux, 0, aux.size()-1);
    }
    
    //to inorder
    void toList(Node root, List<Node> aux){
        if(root==null)
            return;
        toList(root.left, aux);
        aux.add(root);
        toList(root.right, aux);
    }
    
    //following binary traversal to store mid at root and recursively repeating the same
    Node toBalanced(List<Node> aux, int start, int end){
        if(start>end)
            return null;
        int mid=(start+end)/2;
        Node node=aux.get(mid);
        node.left=toBalanced(aux, start, mid-1);
        node.right=toBalanced(aux, mid+1, end);
        return node;
    }
}
